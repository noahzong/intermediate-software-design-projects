 ...
 */
template <uint32_t DIM>
Vector<DIM>::Vector(const double* ptr) noexcept
    : data()
{
 ...
 */
template <uint32_t DIM> Vector<DIM>& Vector<DIM>::operator*=(double rhs)
{
 ...
 *  Scales a copy of this vector by rhs and returns the result.
 */
template <uint32_t DIM> const Vector<DIM> Vector<DIM>::operator*(double rhs) const
{
    return scale(rhs);
}
